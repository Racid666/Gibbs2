...
def expected_shortfall(alpha, returns, var):
    loss_function = lambda x: (x - np.mean(returns)) * norm.pdf(x, np.mean(returns), np.std(returns))
    es, _ = quad(loss_function, -float('inf'), var)
    return (1 / alpha) * es
...
def omega_ratio(returns, threshold):
    # Calculate the expected return above the threshold
    above_threshold = [r for r in returns if r > threshold]
    expected_return_above = np.mean(above_threshold)

    # Calculate the downside deviation below the threshold
    below_threshold = [r for r in returns if r < threshold]
    downside_deviation = np.std(below_threshold)

    # Compute the ratio of the expected return above the threshold to the downside deviation below the threshold
    omega = expected_return_above / downside_deviation
...
def sartino_ratio(returns, threshold):
    # calculate the expected return above the threshold
    above_threshold = [r for r in returns if r > threshold]
    expected_return_above = np.mean(above_threshold)

    # calculate the expected shortfall below the threshold
    below_threshold = [r for r in returns if r < threshold]
    loss_function = lambda x: (x - np.mean(returns)) * norm.pdf(x, np.mean(returns), np.std(returns))
    es, _ = quad(loss_function, -float('inf'), threshold)
